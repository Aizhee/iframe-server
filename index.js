	...
	return function(req, res, next) {
		...
		function file(filepath /*, stat*/) {
			...
			if (hasNoOrigin && (possibleExtensions.indexOf(x) > -1)) {
				...
				if (injectTag === null && IframeServer.logLevel >= 3) {
...
/**
 * Start a iframe server with parameters given as an object
...
IframeServer.start = function(options) {
	...
	IframeServer.logLevel = options.logLevel === undefined ? 2 : options.logLevel;
	...
	var script = options.injectJS || null;
	...
	if (IframeServer.logLevel === 2) {
		app.use(logger('dev', {
			skip: function (req, res) { return res.statusCode < 400; }
		}));
	// Level 2 or above logs all requests
	} else if (IframeServer.logLevel > 2) {
		app.use(logger('dev'));
	}
	...
	server.addListener('listening', function(/*e*/) {
		...
		if (IframeServer.logLevel > 2 && address.address === "0.0.0.0") {
	...
	// Setup file watcher
	IframeServer.watcher = chokidar.watch(watchPaths, {
		ignored: ignored,
		ignoreInitial: true
	});
...
IframeServer.shutdown = function() {
	var watcher = IframeServer.watcher;
	if (watcher) {
		watcher.close();
	}
	var server = IframeServer.server;
	if (server)
		server.close();
};

module.exports = IframeServer;
